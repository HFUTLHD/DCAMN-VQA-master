...
class DataSet_VQS(Data.Dataset):
    def __init__(self, __C):
        ...
        split_list = __C.SPLIT[__C.RUN_MODE].split('+')
        for split in split_list:
            self.ques_list += json.load(open("datasets/vqa/VQS.json", 'r'))['questions']
        ...
        print('== VQS question Dataset size:', self.data_size)
    ...
    def __getitem__(self, idx):
        ...
        if self.__C.RUN_MODE in ['train']:
            ...
            img_feat_x = img_feat['x'].transpose((1, 0))
            img_feat_iter = proc_img_feat(img_feat_x, self.__C.IMG_FEAT_PAD_SIZE)
            pp = sio.loadmat("datasets/val_ground/COCO_val2014_" + ques['question_id'][:-3].zfill(12) +".jpg_"+ques['question_id'][-1]+".mat")
            ground_mask = pp['ground_mask']
            img_mask = proc_img_mask(ground_mask)  # 640 640
            # a = torch.from_numpy(img_mask).unsqueeze(0)
            # pic = toPIL(a.to(dtype=torch.float32))
            # pic.save('a.jpg')

            img_bbox = img_feat['bbox.npy']  # n,4
            img_feat_bbox = proc_img_bbox(img_bbox)  # 100,4
            # Process question
            ques_ix_iter = proc_ques(ques, self.token_to_ix, self.__C.MAX_TOKEN)
        ...
        return torch.from_numpy(img_feat_iter), \
               ...
               torch.from_numpy(ans_iter), torch.from_numpy(img_mask), torch.from_numpy(img_feat_bbox)


    def __len__(self):
        return self.data_size

def proc_img_mask(img_feat):
    img_feat = np.pad(
        img_feat,
        ((0, 640 - img_feat.shape[0]), (0, 640 - img_feat.shape[1])),
        mode='constant',
        constant_values=0
    )

    return img_feat
def proc_img_bbox(img_feat):
    if img_feat.shape[0] > 100:
        img_feat = img_feat[:100]
    img_feat = np.pad(
        img_feat,
        ((0, 100 - img_feat.shape[0]), (0,0)),
        mode='constant',
        constant_values=0
    )
